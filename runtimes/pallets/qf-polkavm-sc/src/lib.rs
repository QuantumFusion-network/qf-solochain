...
pub mod pallet {
    ...
    pub use frame_support::traits::tokens::nonfungible::Transfer;
    ...
    pub(self) type BlockNumber = u32;
    pub(self) type Header = generic::Header<BlockNumber, BlakeTwo256>;
    pub(self) type Block = generic::Block<Header, UncheckedExtrinsic>;

    use xcq_extension_fungibles::EXTENSION_ID;
    use xcq_extension::XcqResult;
    use xcq_extension::XcqApiExt;
    use xcq_primitives::metadata::Metadata;
    // use xcq::{execute_query as xcq_execute_query, metadata as xcq_metadata};
    ...
    #[derive(Encode, Decode, MaxEncodedLen, TypeInfo)]
    #[scale_info(skip_type_params(T))]
    pub(super) struct TransferData<T: Config> {
        source: T::AccountId,
        dest: T::AccountId,
    }

    #[derive(Encode, Decode)]
    pub(super) struct SCTransferData {
        asset: u64,
        source: [u8; 32],
        dest: [u8; 32],
    }
    ...
    // pub(super) type CurrencyCall<T> = <<T as Config>::Currency as Currency<<T as frame_system::Config>::AccountId>>::Balance;
    // pub type Balance<T> = <T>::Balance;
    ...
    #[pallet::storage]
    pub(super) type TransferAmount<T: Config> = StorageMap<_, Blake2_128Concat, CodeHash<T>, u64>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn execute(
            ...
            // let bal = CurrencyCall::transfer(item, destination)
            let res = T::XcqApiExt::execute_query(raw_blob, input_data).unwrap();
            // match res {
            //     Ok(res) => {
            //         log::info!("Result: {:?}", res);
            //     },
