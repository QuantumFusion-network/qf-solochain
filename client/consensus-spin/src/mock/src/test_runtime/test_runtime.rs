...
use sp_application_crypto::{RuntimeAppPublic, ecdsa, ed25519, sr25519};
...
pub mod storage_key_generator {
    ...
    fn concat_hashes(input: &Vec<&[u8]>) -> String {
        input
            .iter()
            .map(|s| sp_crypto_hashing::twox_128(s))
            .map(hex)
            .collect()
    }

    fn twox_64_concat(x: &[u8]) -> Vec<u8> {
        sp_crypto_hashing::twox_64(x)
            .iter()
            .chain(x.iter())
            .cloned()
            .collect()
    ...
    mod genesis_builder_tests {
        ...
        fn build_genesis_config_with_patch_json_works() {
            ...
            let get_from_storage = |key: &str| -> Vec<u8> {
                storage
                    .top
                    .get(&array_bytes::hex2bytes(key).unwrap())
                    .unwrap()
                    .clone()
            };
