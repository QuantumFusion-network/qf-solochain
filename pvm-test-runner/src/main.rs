...
use polkavm::{
	Config as PolkaVMConfig, Engine, GasMeteringKind, InterruptKind, Module, ModuleConfig,
	ProgramBlob, Reg,
};
...
fn main() {
	...
	if cli.debug {
		...
		let exported_fns =
			blob.exports().map(|e| e.symbol().clone().to_string()).collect::<Vec<_>>();
		...
		for imp in imported_fns {
			if imp.is_some() {
				println!(
					"- {:?}",
					String::from_utf8(imp.expect("no import").as_bytes().to_vec()).unwrap()
				);
			}
		}
		println!("\nExported functions:");
	...
	let gas_cost = module
		.calculate_gas_cost_for(pc)
		.expect(&format!("Failed to calculate gas cost for {:?}", pc));
