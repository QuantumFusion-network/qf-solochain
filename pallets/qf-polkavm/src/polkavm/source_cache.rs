...
impl SourceCache {
	...
	fn read_source_file(&mut self, path: &str) -> Option<String> {
		...
		let filesystem_path = if let Some(relative_path) = path.strip_prefix(HOME_PREFIX) {
			...
			//   "~/.cargo/registry/src/github.com-1ecc6299db9ec823/compiler_builtins-0.1.
			// 91/src/ macros.rs"
			let mut buffer = self.home_path()?;
			...
			let (_, sources_root) =
				sources.iter().find(|(sources_hash, _)| hash.starts_with(sources_hash))?;
	...
	pub fn lookup_source_line(&mut self, path: &str, line: u32) -> Option<&str> {
		...
		let next_offset =
			line_to_offset.get(line.wrapping_add(1)).copied().unwrap_or(contents.len());
