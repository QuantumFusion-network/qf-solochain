...
where
	...
	Secondary: RawMask,
{
	/// The primary mask. This mask is used to mark which secondary masks are
	/// non-empty.
	primary_mask: Primary,

	/// Secondary masks. These contain the actual bits that the `BitMask`
	/// stores.
	...
	Secondary: RawMask,
{
	...
	pub fn index(index: u32) -> BitIndex<Primary, Secondary> {
		...
		BitIndex { index, primary, secondary, _phantom: core::marker::PhantomData }
	}
