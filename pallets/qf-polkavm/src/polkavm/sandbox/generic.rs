...
pub(crate) fn as_bytes(slice: &[usize]) -> &[u8] {
	// SAFETY: Casting a &[usize] into a &[u8] is always safe as `u8` doesn't have
...
// On Linux don't depend on the `libc` crate to lower the number of
// dependencies.
#[cfg(target_os = "linux")]
...
// SAFETY: The ownership of an mmapped piece of memory can be safely transferred
// to other threads.
unsafe impl Send for Mmap {}

// SAFETY: An mmaped piece of memory can be safely accessed from multiple
// threads.
...
impl Mmap {
	...
	pub fn as_slice(&self) -> &[u8] {
		// SAFETY: The pointer is either always valid, or is dangling and the length is
		// zero.
		//
		// The memory might not be mapped as readable, so accessing this slice can still
		// produce a segfault, but this is expected due to the low level nature of
		// this helper, and assuming the signal handler is correct it cannot result in
		// unsoundness, for the same reason as to why the `std::process::abort` is
		// also safe.
	...
	pub fn as_slice_mut(&mut self) -> &mut [u8] {
		...
		// zero.
		//
		// The memory might not be mapped as readable or writable, so accessing this
		// slice can still produce a segfault, but this is expected due to the low
		// level nature of this helper, and assuming the signal handler is correct it
		// cannot result in unsoundness, for the same reason as to why the
		// `std::process::abort` is also safe.
...
impl Default for Mmap {
	fn default() -> Self {
		Self { pointer: core::ptr::NonNull::<u8>::dangling().as_ptr().cast::<c_void>(), length: 0 }
	}
}
...
impl VmCtx {
	...
	pub fn new() -> Self {
		VmCtx {
			...
			program_range: 0..0,
			heap_info: HeapInfo { heap_top: 0, heap_threshold: 0 },
			heap_base: 0,
...
unsafe extern "C" fn syscall_hostcall() -> ! {
	// SAFETY: We were called from the inside of the guest program, so vmctx must be
	// valid.
...
unsafe extern "C" fn syscall_trap() -> ! {
	...
	// SAFETY: We were called from the inside of the guest program, so it's safe to
	// trap.
	trigger_trap(vmctx);
}
...
unsafe fn sbrk(vmctx: &mut VmCtx, pending_heap_top: u64) -> Result<Option<u32>, ()> {
	...
	let size = end - start;
	if size > 0 {
		let guest_memory_base =
			(vmctx as *mut VmCtx).cast::<u8>().offset(-GUEST_MEMORY_TO_VMCTX_OFFSET);
...
impl Sandbox {
			...
			for map in maps {
				...
				match map.kind {
					MapKind::Initialized(ref initialize_with) => {
						...
						// NOTE: We can't use `as_slice_mut` as that would return a slice to the
						// whole chunk of memory,       which would include the vmctx to which
						// we're currently holding a reference.
						unsafe {
							core::ptr::copy(
	...
	fn execute_impl(&mut self, mut args: ExecuteArgs) -> Result<(), ExecutionError<Error>> {
		...
		if let Some(gas) = crate::polkavm::sandbox::get_gas(
			&args,
			self.module.as_ref().and_then(|module| module.gas_metering()),
		) {
		...
		if let Some(entry_point) = args.entry_point {
			...
			// SAFETY: Transmuting an arbitrary lifetime into a 'static lifetime is safe as
			// long as the invariants that the shorter lifetime requires are still
			// upheld.
