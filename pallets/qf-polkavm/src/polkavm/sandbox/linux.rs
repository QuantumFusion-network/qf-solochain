	...
	pub fn new(config: &Config) -> Result<Self, Error> {
		...
		if uffd_available {
			...
			linux_raw::sys_uffdio_api(userfaultfd.borrow(), &mut api).map_err(|error| {
				Error::from(format!("failed to fetch the available userfaultfd features: {error}"))
			})?;
			...
			fn kernel_version(utsname: &linux_raw::new_utsname) -> Option<(u32, u32)> {
				...
				let mut release =
					core::ffi::CStr::from_bytes_until_nul(release).ok()?.to_str().ok()?.split('.');
		...
		match clone(SANDBOX_FLAGS)? {
			Fork::Child => {
				let exit_code =
					if linux_raw::sys_sethostname("localhost").is_err() { 1 } else { 0 };
		...
		Ok(GlobalState { shared_memory: ShmAllocator::new()?, uffd_available, zygote_memfd })
	}
}
...
impl ChildProcess {
		...
		loop {
			...
			if let Err(error) = result {
				if error.errno() == linux_raw::EINTR {
					// Should not happen since we should be blocking all signals while this is
	...
	fn check_status(&mut self, non_blocking: bool) -> Result<ChildStatus, Error> {
		...
		// when cloning the child process, so we'd get an ECHILD error without this
		// flag.
		//
		// (And we're not using __WCLONE since that doesn't work for children which ran
		// execve.)
...
// Here we extract the necessary addresses directly from the zygote binary at
// compile time.
const ZYGOTE_TABLES: (AddressTable, ExtTable) = {
		...
		T: Copy,
	{
		...
		//         and it's `Copy` so it's guaranteed not to drop, so this is always
		// safe.
...
fn prepare_zygote() -> Result<Fd, Error> {
	...
	// The size of the zygote blob is always going to be much less than the size of usize, so this
	// never fails.
...
fn prepare_vmctx() -> Result<(Fd, Mmap), Error> {
	...
	#[allow(clippy::unwrap_used)]
	// The size of VmCtx is always going to be much less than the size of usize, so this never
	// fails.
	let length_aligned =
...
unsafe fn child_main(uid_map: &str, gid_map: &str, fds: ChildFds) -> Result<(), Error> {
	...
	if !cfg!(polkavm_dev_debug_zygote) {
		...
		// Disable the 'setgroups' syscall. Probably unnecessary since we'll do it
		// though seccomp anyway, but just in case. (See CVE-2014-8989 for more
		// details.)
		let proc_self =
			linux_raw::sys_open(cstr!("/proc/self"), linux_raw::O_CLOEXEC | linux_raw::O_PATH)?;
		...
		// Set up UID and GID maps. This can only be done once, so if we do it here
		// we'll block the possibility of doing it later.
		let fd = linux_raw::sys_openat(
			proc_self.borrow(),
			cstr!("gid_map"),
	...
	// to this FD. Unfortunately writing to a closed pipe generates a SIGPIPE
	// signal, and normally that'd be fine since we ignore SIGPIPE in the child,
	// however when running under ptrace this will also stop the process and return
	// a "trapped" status to the host, even if the SIGPIPE would normally be
	// ignored!
	//
	...
	linux_raw::sys_setrlimit(
		...
		&linux_raw::rlimit { rlim_cur: max_memory, rlim_max: max_memory },
	)?;
	linux_raw::sys_setrlimit(
		linux_raw::RLIMIT_STACK,
		&linux_raw::rlimit { rlim_cur: 16 * 1024, rlim_max: 16 * 1024 },
	)?;

	linux_raw::sys_setrlimit(
		linux_raw::RLIMIT_NPROC,
		&linux_raw::rlimit { rlim_cur: 1, rlim_max: 1 },
	)?;
	linux_raw::sys_setrlimit(
		linux_raw::RLIMIT_FSIZE,
		&linux_raw::rlimit { rlim_cur: 0, rlim_max: 0 },
	)?;
	linux_raw::sys_setrlimit(
	...
	// This should never happen, but since the never type is still unstable let's
	// return normally.
	Ok(())
}
...
impl super::Sandbox for Sandbox {
	...
	fn downcast_module(module: &Module) -> &CompiledModule<Self> {
		let CompiledModuleKind::Linux(module) = module.compiled_module() else { unreachable!() };
		module
	}
	...
	fn spawn(global: &Self::GlobalState, config: &SandboxConfig) -> Result<Self, Error> {
		...
		let maps = std::fs::read(format!("/proc/{}/maps", child_pid)).map_err(|error| {
			Error::from_errno("failed to read child's maps", error.raw_os_error().unwrap_or(0))
		})?;
			...
			for (chunk, vm_map) in program.memory_map.iter().zip(vm_maps.iter_mut()) {
				...
				*vm_map = VmMap {
					...
					protection: linux_raw::PROT_READ |
						if chunk.is_writable { linux_raw::PROT_WRITE } else { 0 },
...
impl Sandbox {
	...
	fn wait(&mut self) -> Result<Interrupt, Error> {
		...
		'outer: loop {
			...
			let spin_target =
				if self.module.as_ref().map_or(false, |module| module.is_step_tracing()) {
					128
				} else {
					0
				};
