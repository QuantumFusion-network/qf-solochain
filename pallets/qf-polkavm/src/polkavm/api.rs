...
use polkavm_common::{
	abi::{MemoryMap, MemoryMapBuilder, VM_ADDR_RETURN_TO_HOST},
	cast::cast,
	program::{
...
use crate::polkavm::{
	Gas, ProgramCounter,
	config::{BackendKind, Config, GasMeteringKind, ModuleConfig, SandboxKind},
	error::{Error, bail, bail_static},
	interpreter::{InterpretedInstance, InterpretedModule},
	utils::{GuestInit, InterruptKind},
};
...
impl Module {
	...
	/// Creates a new module by deserializing the program from the given
	/// `bytes`.
	...
	pub fn from_blob(
		...
		let instruction_set =
			RuntimeInstructionSet { allow_sbrk: config.allow_sbrk, is_64_bit: blob.is_64_bit() };
		...
		let interpreted_module =
			if engine.interpreter_enabled { Some(InterpretedModule::new(init)?) } else { None };
	...
	pub fn instantiate(&self) -> Result<RawInstance, Error> {
		...
		Ok(RawInstance { module: self.clone(), backend, crosscheck_instance })
	}
	...
	/// A slice which contains pairs of PolkaVM bytecode offsets and native
	/// machine code offsets.
	///
	/// This makes it possible to map a position within the guest program into
	/// the exact range of native machine code instructions.
	///
	/// The returned slice has as many elements as there were instructions in
	/// the original guest program, plus one extra to make it possible to
	/// figure out the length of the machine code corresponding to the very
	/// last instruction.
	///
	/// This slice is guaranteed to be sorted, so you can binary search through
	/// it.
	///
	/// Will return `None` when running under an interpreter.
	/// Mostly only useful for debugging.
	...
	/// Calculates the gas cost for a given basic block starting at
	/// `code_offset`.
	///
...
impl RawInstance {
	...
	///
	/// Note that this being zero doesn't necessarily mean that the execution
	/// ran out of gas, if the program ended up consuming *exactly* the amount
	/// of gas that it was provided with!
	pub fn gas(&self) -> Gas {
	...
	///
	/// This is where the program will resume execution when
	/// [`RawInstance::run`] is called.
	...
	/// Sets the accessible region of the aux data, rounded up to the nearest
	/// page size.
	...
	/// Returns whether a given chunk of memory is accessible through
	/// [`read_memory_into`](Self::read_memory_into)/
	...
	///
	/// When dynamic paging is enabled calling this can be used to resolve a
	/// segfault. It can also be used to preemptively initialize pages for
	/// which no segfault is currently triggered.
	pub fn write_memory(&mut self, address: u32, data: &[u8]) -> Result<(), MemoryAccessError> {
		...
		let result =
			access_backend!(self.backend, |mut backend| backend.write_memory(address, data));
		if let Some(ref mut crosscheck) = self.crosscheck_instance {
	...
	pub fn read_memory(&self, address: u32, length: u32) -> Result<Vec<u8>, MemoryAccessError> {
		...
		// Since `read_memory_into_slice` returns a `&mut [u8]` we can be sure it
		// initialized the buffer we've passed to it, as long as it's actually the
		// same buffer we gave it.
		assert_eq!(slice.as_ptr(), pointer);
		assert_eq!(slice.len(), cast(length).to_usize());

		#[allow(unsafe_code)]
		// SAFETY: `read_memory_into_slice` initialized this buffer, and we've verified this with
		// `assert`s.
		unsafe {
	...
	///
	/// `address` must be greater or equal to 0x10000 and `address + length`
	/// cannot be greater than 0x100000000. If `length` is zero then this call
	...
	pub fn zero_memory(&mut self, address: u32, length: u32) -> Result<(), MemoryAccessError> {
		...
		if address < 0x10000 {
			return Err(MemoryAccessError::OutOfRangeAccess { address, length: u64::from(length) });
		}
		...
		let result =
			access_backend!(self.backend, |mut backend| backend.zero_memory(address, length));
		if let Some(ref mut crosscheck) = self.crosscheck_instance {
	...
	///
	/// `address` must be a multiple of the page size. The value of `length`
	/// will be rounded up to the nearest multiple of the page size. If
	/// `length` is zero then this call has no effect and will always succeed.
	...
	/// A convenience function which sets up a fuction call according to the
	/// default ABI.
	///
	/// This function will:
	...
	///
	/// Will panic if marshalling `args` through the FFI boundary requires too
	/// many registers.
	...
	/// Extracts a return value from the argument registers according to the
	/// default ABI.
	///
		...
		FnResult: crate::polkavm::linker::FuncResult,
	{
		...
		FnResult::_get(self.module().blob().is_64_bit(), || {
			let value =
				access_backend!(self.backend, |backend| backend.reg(Reg::ARG_REGS[output_count]));
			output_count += 1;
			value
		})
	}
