...
pub enum InterruptKind {
	...
	///
	/// Requires dynamic paging to be enabled with
	/// [`ModuleConfig::set_dynamic_paging`](crate::polkavm::ModuleConfig::set_dynamic_paging),
	/// otherwise is never emitted.
	Segfault(Segfault),

	/// The execution ran out of gas.
	///
	/// Requires gas metering to be enabled with
	/// [`ModuleConfig::set_gas_metering`](crate::polkavm::ModuleConfig::set_gas_metering),
	/// otherwise is never emitted.
	NotEnoughGas,

	/// Executed a single instruction.
	///
	/// Requires execution step-tracing to be enabled with
	/// [`ModuleConfig::set_step_tracing`](crate::polkavm::ModuleConfig::set_step_tracing),
	/// otherwise is never emitted.
	Step,
}
