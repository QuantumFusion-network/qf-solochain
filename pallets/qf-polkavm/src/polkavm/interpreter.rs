...
// Define a custom trait instead of just using `Into<RegImm>` to make sure this
// is always inlined.
trait IntoRegImm {
	fn into(self) -> RegImm;
}
...
impl InterpretedModule {
	pub fn new(init: GuestInit) -> Result<Self, Error> {
		...
		Ok(InterpretedModule { ro_data, rw_data: init.rw_data.into() })
	}
}
...
impl BasicMemory {
	...
	fn sbrk(&mut self, module: &Module, size: u32) -> Option<u32> {
			...
			cast(memory_map.rw_data_address()).to_usize() + self.rw_data.len()
		{
			...
			.unwrap() - cast(memory_map.rw_data_address()).to_usize();
...
impl Page {
	fn empty(page_size: u32) -> Self {
		...
		Page { data: page.into(), is_read_only: false }
	}
}
...
impl InterpretedInstance {
	...
	pub fn zero_memory(&mut self, address: u32, length: u32) -> Result<(), MemoryAccessError> {
		if !self.module.is_dynamic_paging() {
			let Some(slice) =
				self.basic_memory.get_memory_slice_mut::<true>(&self.module, address, length)
			else {
				return Err(MemoryAccessError::OutOfRangeAccess {
					address,
	...
	///
	/// Unlike jumps from within the program these can start execution anywhere
	/// to support suspend/resume of the VM.
	...
	fn compile_out_of_range_stub(&mut self) {
		...
		let gas_cost =
			if self.module.gas_metering().is_some() { crate::polkavm::gas::trap_cost() } else { 0 };
...
impl<'a> Visitor<'a> {
	...
	fn load<T: LoadTy, const DEBUG: bool, const IS_DYNAMIC: bool>(
		...
		let value = if !IS_DYNAMIC {
			let Some(slice) =
				self.inner.basic_memory.get_memory_slice(&self.inner.module, address, length)
			else {
				if DEBUG {
					log::debug!(
			...
			if page_address_lo == page_address_hi {
				...
				let mut iter =
					self.inner.dynamic_memory.pages.range(page_address_lo..=page_address_hi);
				let lo = iter.next();
				let hi = iter.next();
	...
	fn store<T: StoreTy, const DEBUG: bool, const IS_DYNAMIC: bool>(
		...
		if !IS_DYNAMIC {
			...
			if page_address_lo == page_address_hi {
				...
				let mut iter =
					self.inner.dynamic_memory.pages.range_mut(page_address_lo..=page_address_hi);
				let lo = iter.next();
				let hi = iter.next();
	...
	fn jump_indirect_impl<const DEBUG: bool>(
		...
		let Some(target) = self.inner.module.jump_table().get_by_address(dynamic_address) else {
			if DEBUG {
				log::trace!(
					"Indirect jump to dynamic address {dynamic_address}: invalid (bad jump table index)"
				);
			}
...
fn transmute_reg(value: u32) -> Reg {
	...
	// SAFETY: The `value` passed in here is always constructed through `reg as u32`
...
macro_rules! handle_unresolved_branch {
	($debug:expr, $visitor:ident, $s1:ident, $s2:ident, $tt:ident, $tf:ident, $name:ident) => {{
		...
		let target_true =
			$visitor.inner.resolve_jump::<DEBUG>($tt).unwrap_or(TARGET_INVALID_BRANCH);
