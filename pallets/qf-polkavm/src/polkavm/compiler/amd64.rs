...
use crate::polkavm::{
	compiler::{ArchVisitor, Bitness, CompilerBitness, SandboxKind},
	config::GasMeteringKind,
	sandbox::Sandbox,
	utils::RegImm,
};

/// The register used for the embedded sandbox to hold the base address of the
/// guest's linear memory.
const GENERIC_SANDBOX_MEMORY_REG: NativeReg = AUX_TMP_REG;

/// The register used for the linux sandbox to hold the address of the VM
/// context.
	...
	R: NonZero,
{
	if let Some(offset) = asm.get_label_origin_offset(label) {
		...
		//   fc                   cld
		//   ff ff                invalid
		// The `cld` here is harmless, and the 'ff' is the opcode shared by single
		// register 'inc/dec/call/jmp/push' instructions, however the /7 opext it has
		// here is currently undefined so this will trap. Same as for gas, this is
		// technically a forward compatibility hazard.
	...
	B: CompilerBitness,
{
	...
	fn compare_reg_imm(&mut self, d: RawReg, s1: RawReg, s2: u32, condition: Condition) {
		...
		let asm = if d != s1 { asm.push(xor((RegSize::R32, d, d))) } else { asm.push_none() };
	...
	fn div_rem(
		...
		// The division instruction always accepts the dividend and returns the result
		// in rdx:rax. This isn't great because we're using these registers for the
		// VM's registers, so we need to do all of this in such a way that those won't
	...
	// This is a slower memset implementation when we're running with gas metering
	// enabled and we don't have enough gas to finish running the whole memset.
	pub(crate) fn emit_memset_trampoline(&mut self) {
		...
		// Grab the amount of gas we have (this will always be negative), and zero the
		// gas counter. (We assume the memset will consume all of the gas.)
		...
		// Set the final counter to how much bytes will be remaining when we run out of
		// gas.
		...
		// We've successfully finished memset without page faulting, so we can run out
		// of gas.
		self.save_registers_to_vmctx();
	...
	pub(crate) fn emit_gas_metering_stub(&mut self, kind: GasMeteringKind) {
		...
		if matches!(kind, GasMeteringKind::Sync) {
			...
			// This will jump two bytes backwards to "3f00", and 3f corresponds to the AAS
			// instruction which is invalid in 64-bit, so it will trap with an SIGILL.
			//
			// Note that this is technically a forward-compatibility hazard as this opcode
			// could arguably be reused for something in the future.
			assert_eq!(
			...
			// Offset to bring us from where the trap will trigger to the beginning of the
			// stub.
	...
	pub fn sbrk(&mut self, dst: RawReg, size: RawReg) {
		...
		// If it was less or equal to the threshold then no extra action is necessary
		// (bump only!).
		self.push(jcc_label8(Condition::BelowOrEqual, label_bump_only));

		// The new top-of-the-heap pointer crossed the threshold, so more involved
		// handling is necessary. We'll either allocate new memory, or return a null
		// pointer.
		self.push(mov(RegSize::R64, TMP_REG, dst));
		self.call_to_label(sbrk_label);
		self.push(mov(RegSize::R32, dst, TMP_REG));
		// Note: `dst` can be zero here, which is why we do the pointer bump from within
		// the handler.
	...
	pub fn memset(&mut self) {
		...
		match self.gas_metering {
			...
			Some(GasMeteringKind::Sync) => {
				...
				self.asm
					.push(sub((RegSize::R64, Self::vmctx_field(S::offset_table().gas), count)));
	...
	pub fn on_signal_trap(
		...
		match trap_kind {
			TrapKind::NotEnoughGas => {
				...
				// If we restart we want to check the gas again, so set the address to point
				// there.
				vmctx
					.next_native_program_counter
					.store(compiled_module.native_code_origin + offset, Ordering::Relaxed);

				// Also set the logical program counter, in case the user wants to stash the
				// location somewhere and continue some other time.
				...
				// The gas counter is now negative; refund the amount of gas consumed so that
				// it's positive again.
			...
			TrapKind::Memset { kind } => {
				...
				// We can only be here if dynamic page faulting is disabled, so this situation
				// is unrecoverable.
				vmctx.next_native_program_counter.store(0, Ordering::Relaxed);
