...
use crate::polkavm::{
	api::RuntimeInstructionSet,
	config::{GasMeteringKind, ModuleConfig, SandboxKind},
	gas::GasVisitor,
	mutex::Mutex,
	sandbox::{Sandbox, SandboxInit, SandboxProgram},
	utils::{FlatMap, GuestInit},
};
...
///
/// This needs to be at least 0x800000000000 on modern CPUs, but ideally should
/// have the most significant bit set to be future proof.
///
/// Why 0x800000000000? This constant is 48-bit (a single '1' followed by 47
/// '0's) which is how many bits of virtual address space most modern CPUs
/// support, and we deliberately want to have an address which is bigger than
/// this.
///
/// If the CPU encounters a jump instruction, and that instruction tells it to
/// go to an address which fits into 48 bits, then that might be a jump to
/// somewhere valid, so the CPU has no choice but to execute it, and clobber the
/// instruction pointer with the target address in the process.
///
/// However, if it is a jump to an address that does *not* fit into 48 bits then
/// the CPU can immediately generate a page fault without even trying to jump
/// there, leaving the original value of the instruction pointer alone, which is
	...
	B: CompilerBitness,
{
		...
		S: Sandbox,
	{
		...
		let native_code_origin =
			crate::polkavm::sandbox::SandboxAddressSpace::native_code_origin(&address_space);
		...
		S: Sandbox,
	{
		...
		if self.gas_metering.is_some() {
			...
			for (&native_code_offset, &cost) in
				gas_metering_stub_offsets.iter().zip(gas_cost_for_basic_block.iter())
			{
	...
	fn after_instruction<const KIND: usize>(&mut self, program_counter: u32, args_length: u32) {
		...
		if cfg!(debug_assertions) && !self.step_tracing {
			let offset =
				self.program_counter_to_machine_code_offset_list.last().unwrap().1 as usize;
	...
	S: Sandbox,
{
	...
	pub fn program_counter_by_native_code_offset(
			...
			.binary_search_by_key(&offset, |&(_, native_offset)| u64::from(native_offset))
		{
			...
			Err(index) => {
				if !strict &&
					index > 0 && index < self.program_counter_to_machine_code_offset_list.len()
				{
					index - 1
				} else {
					return None;
