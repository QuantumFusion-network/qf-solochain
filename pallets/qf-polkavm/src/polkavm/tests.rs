...
use polkavm_common::{
	...
	program::{BlobLen, DefaultInstructionSet, Reg::*, asm},
...
fn jump_into_middle_of_basic_block_from_within(engine_config: Config) {
	...
	let instructions = {
		...
		let instructions: Vec<_> =
			module.blob().instructions(DefaultInstructionSet::default()).collect();
...
fn dynamic_paging_basic(mut engine_config: Config) {
	...
	// Running the program again produces no more segfaults, since everything is
	// faulted already.
	instance.set_next_program_counter(offsets[0]);
	match_interrupt!(instance.run().unwrap(), InterruptKind::Finished);
}
...
fn dynamic_paging_protect_memory(mut engine_config: Config) {
	...
	if engine_config.crosscheck() {
		// TODO: This is currently broken due to a different stepping behavior when page
		// faults are involved.
		return;
	}
...
fn dynamic_paging_initialize_multiple_pages(mut engine_config: Config) {
	...
	builder.add_export_by_basic_block(0, b"main");
	builder.set_code(
		&[asm::load_i32(Reg::A0, 0x10004), asm::load_i32(Reg::A1, 0x10004 + page_size), asm::ret()],
		&[],
	);
...
fn dynamic_paging_change_program_counter_during_segfault(mut engine_config: Config) {
	...
	builder.add_export_by_basic_block(0, b"main");
	builder.set_code(
		&[asm::store_imm_u32(0x10000, 1), asm::ret(), asm::store_imm_u32(0x11000, 2), asm::ret()],
		&[],
	);
...
fn dynamic_paging_instantiate_on_another_thread(mut engine_config: Config) {
...

	{
		...
		let mut instance =
			std::thread::spawn(move || module.instantiate()).join().unwrap().unwrap();
		instance.set_reg(Reg::RA, crate::polkavm::RETURN_TO_HOST);
		instance.set_next_program_counter(offsets[0]);
		instance.set_gas(1000000);
...
fn doom(config: Config, elf: &'static [u8]) {
	...
	linker
		.define_typed(
			...
			|caller: Caller<State>, pointer: u32, offset: u32, length: u32| -> Result<(), String> {
				let chunk =
					DOOM_WAD.get(offset as usize..offset as usize + length as usize).ok_or_else(
	...
	let mut state = State { frame: Vec::new(), frame_width: 0, frame_height: 0 };
	...
	//             pixel[3] = 0xff;
	//         }
	//         let filename = format!("/tmp/doom-frames/doom_{:05}.tga",
	// nth_frame);         image::save_buffer(filename, &state.frame,
	// state.frame_width, state.frame_height, image::ColorType::Rgba8).
	// unwrap();     } }
}
...
fn access_memory_from_host(config: Config) {
	...
	for (range, is_read_only) in list {
		...
		// Partial writes should not clobber the memory region, so do the failing writes
		// first.
...
fn memset_with_dynamic_paging(mut config: Config) {
	...
	instance.prepare_call_typed(
		offsets[0],
		(memory_map.rw_data_range().start + page_size - 1, 0x1234567a, 4),
	);
	let segfault = expect_segfault(instance.run().unwrap());
...
fn test_basic_debug_info(raw_blob: &'static [u8]) {
	...
	let entry_point =
		program.exports().find(|export| export == "read_u32").unwrap().program_counter();
