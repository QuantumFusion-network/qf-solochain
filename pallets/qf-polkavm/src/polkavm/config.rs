...
impl Config {
	...
	/// Creates a new default configuration and seeds it from the environment
	/// variables.
		...
		#[cfg(feature = "std")]
		{
			...
			if let Some(value) = env_usize("POLKAVM_LRU_CACHE_SIZE")? {
				config.lru_cache_size =
					if value > u32::MAX as usize { u32::MAX } else { value as u32 };
			}
		}
	...
	///
	/// Corresponding environment variable: `POLKAVM_BACKEND` (`auto`,
	...
	///
	/// Corresponding environment variable: `POLKAVM_SANDBOX` (`auto`, `linux`,
	/// `generic`)
	...
	///
	/// This will run an interpreter alongside the recompiler and cross-check
	/// their execution.
	///
	/// Should only be used for debugging purposes and *never* enabled by
	/// default in production.
	///
	/// Default: `false`
	///
	/// Corresponding environment variable: `POLKAVM_CROSSCHECK` (`false`,
	/// `true`)
	...
	///
	/// This should NEVER be used in production unless you know what you're
	/// doing.
	///
	/// Default: `false`
	///
	/// Corresponding environment variable: `POLKAVM_ALLOW_EXPERIMENTAL`
	/// (`true`, `false`)
	...
	/// Sets the number of worker sandboxes that will be permanently kept alive
	/// by the engine.
	///
	/// This doesn't limit the number of instances that can be instantiated at
	/// the same time; it will just tell the engine how many sandboxes should
	/// be cached between instantiations.
	///
	/// For the Linux sandbox this will decide how many worker processes are
	/// kept alive.
	///
	/// This only has an effect when using a recompiler. For the interpreter
	...
	/// Returns the number of worker sandboxes that will be permanently kept
	/// alive by the engine.
	...
	///
	/// Enabling this increases the minimum system requirements of the
	/// recompiler backend:
	///  - At least Linux 6.7 is required.
	///  - Unpriviledged `userfaultfd` must be enabled (`/proc/sys/vm/unprivileged_userfaultfd` must
	///    be set to `1`).
	///
	/// Default: `false`
	...
	///
	/// When set to `true` calling [`Module::new`](crate::polkavm::Module::new)
	/// or [`Module::from_blob`](crate::polkavm::Module::from_blob) will return
	/// an already compiled module if such already exists.
	///
	/// Requires the `module-cache` compile time feature to be enabled,
	...
	///
	/// Requires the `module-cache` compile time feature and caching to be
	/// enabled, otherwise has no effect.
	///
	/// When the size of the LRU cache is non-zero then modules that are dropped
	/// will be added to the LRU cache, and will be reused if a compilation of
	/// the same program is triggered.
	///
	/// Default: `0`
	///
...
pub enum GasMeteringKind {
	...
	/// synchronous gas metering, but will only periodically and asynchronously
	/// check whether we still have gas remaining while the program is running.
	///
	/// With asynchronous gas metering the program can run slightly longer than
	/// it would otherwise, and the exact point *when* it is interrupted is not
	/// deterministic, but whether the computation as a whole finishes under a
	/// given gas limit will still be strictly enforced and deterministic.
	///
	/// This is only a hint, and the VM might still fall back to using
	/// synchronous gas metering if asynchronous metering is not available.
	Async,
}
...
impl ModuleConfig {
	...
	///
	/// [`Config::allow_dynamic_paging`] also needs to be `true` for dynamic
	...
	///
	/// When enabled
	/// [`InterruptKind::Step`](crate::polkavm::InterruptKind::Step) will be
	/// returned by [`RawInstance::run`](crate::polkavm::RawInstance::run) for
	...
	///
	/// When enabled sbrk instruction is not allowed it will lead to a trap,
	/// otherwise sbrk instruction is emulated.
	///
	/// Default: `true`
	...
	///
	/// This introduces extra overhead as every time a module compilation is
	/// triggered the hash of the program must be calculated, and in general it
	/// is faster to recompile a module from scratch rather than compile its
	/// hash.
	///
	/// Default: `true`
