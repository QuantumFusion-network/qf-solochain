...
fn test_calculate_optimal_bin_config() {
	assert_eq!(
		...
		AllocatorBinConfig { mantissa_bits: 8, bin_count: 3072 }
	);
	...
	assert_eq!(
		...
		AllocatorBinConfig { mantissa_bits: 7, bin_count: 1664 }
	);
}
...
where
	Size: SizeT,
{
	pub const EMPTY: GenericAllocation<Size> =
		GenericAllocation { node: EMPTY, offset: Size::ZERO, size: Size::ZERO };
	...
	C: AllocatorConfig,
{
	...
	fn insert_free_node(&mut self, offset: C::Size, size: C::Size) -> Option<u32> {
		...
		// Get the bin index; round down to make sure the node's size is at least as big
		// as what the bin expects.
	...
	pub fn alloc(&mut self, size: C::Size) -> Option<GenericAllocation<C::Size>> {
		...
		// Calculate the minimum bin to fit this allocation; round up in case the size
		// doesn't match the bin size exactly.
		...
		let (bin, node) = if let Some(bin) = self.bins_with_free_space.find_first(min_bin) {
			...
			// No such bin exists; let's try rounding down and see if maybe we can find an
			// oversized region in the previous bin.
		...
		// If we haven't allocated all of the free space then add what's remaining back
		// for later use.
		...
		if let Some(new_free_node) = self.insert_free_node(offset + size, remaining_free_pages) {
			...
			let next_by_address =
				replace(&mut self.nodes[node as usize].next_by_address, new_free_node);
...
pub mod tests {
		...
		C: super::AllocatorConfig,
	{
		pub fn new(size: C::Size) -> Self {
			Self { allocator: GenericAllocator::<C>::new(size), allocations: HashMap::new() }
		}
