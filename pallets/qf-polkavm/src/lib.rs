...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn execute(
			...
			let mut state = State {
				addresses: [contract_address.clone(), who.clone(), to].to_vec(),
				balances: [value].to_vec(),
				user_data,
				...
				max_log_len,
				transfer: |from: T::AccountId, to: T::AccountId, value: BalanceOf<T>| -> u64 {
				...
				print: |m: Vec<u8>| -> u64 {
					...
					let mut msg_log = alloc::string::String::from("polkavm: ");
					msg_log.push_str(msg.as_ref());
